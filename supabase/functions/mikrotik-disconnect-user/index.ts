...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Connected to MikroTik at ${settings.host}:${settings.port}`);

    let disconnected = false;

    // Try to disconnect PPPoE user
    try {
      const pppoeActive = await client.command("/ppp/active/print", { name: username });
      if (pppoeActive.length > 0) {
        const userId = pppoeActive[0][".id"];
        await client.command("/ppp/active/remove", { ".id": userId });
        console.log(`Disconnected PPPoE user: ${username}`);
        disconnected = true;
      }
    } catch (error) {
      console.log(`Not a PPPoE user or error: ${error}`);
    }

    // Try to disconnect Hotspot user if not PPPoE
    if (!disconnected) {
      try {
        const hotspotActive = await client.command("/ip/hotspot/active/print", { user: username });
        if (hotspotActive.length > 0) {
          const userId = hotspotActive[0][".id"];
          await client.command("/ip/hotspot/active/remove", { ".id": userId });
          console.log(`Disconnected Hotspot user: ${username}`);
          disconnected = true;
        }
      } catch (error) {
        console.log(`Not a Hotspot user or error: ${error}`);
      }
    }
    ...
    if (!disconnected) {
      return new Response(
        JSON.stringify({ success: false, message: "User not found online" }),
        { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    return new Response(
      JSON.stringify({ success: true, message: `User ${username} disconnected successfully` }),
